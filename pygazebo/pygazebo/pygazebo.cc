...
class RayObservation {
 public:
  RayObservation(ignition::math::Angle angle_min, ignition::math::Angle angle_max, size_t angle_resolution, size_t range_min, size_t range_max, size_t range_resolution, size_t num, double* data): angle_min_(angle_min), angle_max_(angle_max), angle_resolution_(angle_resolution), range_min_(range_min), range_max_(range_max), range_resolution_(range_resolution), num_(num), data_(data) {}

  ignition::math::Angle angle_min() const { return angle_min_; }
  ignition::math::Angle angle_max() const { return angle_max_; }
  size_t angle_resolution() const { return angle_resolution_; }
  size_t range_min() const { return range_min_; }
  size_t range_max() const { return range_max_; }
  size_t range_resolution() const { return range_resolution_; }
  size_t num() const { return num_; }
  double* data() { return data_; }
 
 private:
   ignition::math::Angle angle_min_, angle_max_;
   size_t angle_resolution_, range_min_, range_max_, range_resolution_, num_;
   double* data_;
};   
...
class Model {
 ...
 public:
  ...
  Twist GetTwist() const {
    ...
    return std::make_tuple(
        std::make_tuple(linear.X(), linear.Y(), linear.Z()),
        std::make_tuple(angular.X(), angular.Y(), angular.Z()));
  }
  ...
  void SetTwist(const Twist& twist) {
    auto linear = std::get<0>(twist);
    auto angular = std::get<1>(twist);
    ignition::math::Vector3d linear3d(std::get<0>(linear),
                                      std::get<1>(linear),
                                      std::get<2>(linear));
    ignition::math::Vector3d angular3d(std::get<0>(angular),
                                       std::get<1>(angular),
                                       std::get<2>(angular));
    model_->SetWorldTwist(linear3d, angular3d);
  }
};
...
class Agent : public Model {
 private:
  ...
  std::map<std::string, gazebo::sensors::RaySensorPtr> rays_;
 ...
 public:
  ...
  RayObservation GetRayObservation(const std::string& sensor_scope_name) {
    auto it = rays_.find(sensor_scope_name);
    
    if (it == rays_.end()) {
	gazebo::sensors::SensorManager* mgr = gazebo::sensors::SensorManager::Instance();
	
	gazebo::sensors::RaySensorPtr sensor = std::dynamic_pointer_cast<gazebo::sensors::RaySensor>( mgr->GetSensor(sensor_scope_name));

	if (!sensor) {
	  std::cerr << "unable to find sensor: " << sensor_scope_name
                  << std::endl;
	}
        std::cerr << "able to find sensor: " << sensor_scope_name
                  << std::endl;
	auto ret = rays_.insert(std::make_pair(sensor_scope_name, sensor));
	it = ret.first;
      }

    auto sensor = it->second;
    std::vector<double> result;
    sensor->Ranges(result);
    size_t asize = result.size();
    double *data_ = new double[asize];
    if (!result.empty()) 
    {
    ...
    return RayObservation(sensor->AngleMin(), sensor->AngleMax(), sensor->AngleResolution(), sensor->RangeMin(), sensor->RangeMax(), sensor->RangeResolution(), sensor->RangeCount(), data_);
  }
...
class World {
 ...
 public:
  ...
  std::vector<std::unique_ptr<Agent>> GetAgents(const std::string& name) {
    if (name.empty()) {
      std::vector<std::unique_ptr<Agent>> Agents_Model;
      int i = 0;
      for (auto model : world_->Models()) {
        if (model->GetJointCount() > 1) {
          Agents_Model.push_back(std::make_unique<Agent>(model));
          ++i;
        }
      }
      return Agents_Model;
    ...
    std::vector<std::unique_ptr<Agent>> Agents_Model;
    Agents_Model.push_back(std::make_unique<Agent>(world_->ModelByName(name)));
    return Agents_Model;
  }
  ...
  const float GetSimTime() { 
       gazebo::physics::State state;
       return state.GetSimTime().Float();}
  
  const float GetWallTime() {
       gazebo::common::Time time;
       return time.GetWallTime().Float();}
};
...
PYBIND11_MODULE(pygazebo, m) {
  ...
  py::class_<World>(m, "World")
      ...
      .def("get_agent",
           ...
           py::arg("name") = "")
      .def("get_agents", &World::GetAgents, "Get an agents by name", py::arg("name") = "")
      .def(
      ...
      .def("get_wall_time", &World::GetWallTime, "show current time in wall")
      .def("get_sim_time", &World::GetSimTime, "show current time in simulation");
  ...
  py::class_<Model>(m, "Model")
      ...
      .def("get_twist",
           &Model::GetTwist,
           "Get ((linear_x, linear_y, linear_z), (angular_x, angualr_y, angular_z)) of the agent")
      .def("set_pose",
           &Model::SetPose,
           "Set ((x,y,z), (roll, pitch, yaw)) of the agent")
      .def("set_twist",
           &Model::SetTwist,
           "Set ((linear_x, linear_y, linear_z), (angular_x, angualr_y, angular_z))");
  ...
  py::class_<RayObservation>(m, "RayObservation", py::buffer_protocol())
      .def_buffer([](RayObservation& m) -> py::buffer_info {
        return py::buffer_info(m.data(),
                               sizeof(double),
                               py::format_descriptor<double>::format(),
                               1,
                               {m.num()},
                               // memory layout for laser data
                               {sizeof(double)});
      })
      .def("get_data", &RayObservation::data, "get laser data");
  ...
  py::class_<Agent, Model>(m, "Agent")
      ...
      .def("get_ray_observation",
           &Agent::GetRayObservation,
           py::arg("sensor_scope_name"))
