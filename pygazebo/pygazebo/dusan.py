...
while is_all_escape(evader_flag):
    ...
    for i in range(evader_num):
        if evader_flag[i] is True:
            ...
            agents[i+pursuer_num].set_twist(((V_e[i]*math.cos(evader[i][1][2])*ts, V_e[i]*math.sin(evader[i][1][2])*ts, 0), (0, 0, W_e[i]*ts)))
        print(agents[i+pursuer_num].get_twist())
        # print(((V_e[i]*math.cos(evader[i][1][2])*ts, V_e[i]*math.sin(evader[i][1][2])*ts, 0), (0, 0, W_e[i]*ts)))

    for j in range(pursuer_num):
        if pursuer_flag[j] is True:
            agents[j].set_twist(((0, 0, 0), (0, 0, 0)))
        else:
            agents[j].set_twist(((V_p[j]*math.cos(pursuer[j][1][2]*ts), V_p[j]*math.sin(pursuer[j][1][2])*ts, 0), (0, 0, W_p[j]*ts)))
        print(agents[j].get_twist())
        # print(((V_p[j]*math.cos(pursuer[j][1][2]*ts), V_p[j]*math.sin(pursuer[j][1][2])*ts, 0), (0, 0, W_p[j]*ts)))
